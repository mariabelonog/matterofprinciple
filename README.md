...
package.json	Список зависимостей и скрипты (npm run dev, npm run build). Сердце любого Node-проекта.
package-lock.json	Автоматически генерируемый lock-файл — фиксирует точные версии всех пакетов. В git он есть намеренно: гарантирует, что у всех одинаковые версии зависимостей.
tsconfig.json	Настройки TypeScript компилятора: что проверять, какие пути (алиас @/*), какой стандарт JS на выходе.
next.config.ts	Конфигурация Next.js — можно включать экспериментальные фичи, настраивать редиректы и т.д. У нас почти пустой.
eslint.config.mjs	Правила линтера — проверяет качество кода автоматически при сборке.
postcss.config.mjs	Нужен Tailwind CSS — обрабатывает CSS-файлы через PostCSS pipeline.
